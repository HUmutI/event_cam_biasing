...
def make_event_image(events, resolution):
    ...
    # Grab a numpy array of shape (N,4): [ts, x, y, polarity]
    arr = events.numpy()  # direct C→NumPy bridge :contentReference[oaicite:0]{index=0}
...
def main():
    ...
    bias_types = [
        getattr(DAVIS.Davis346BiasCF, attr)
        for attr in dir(DAVIS.Davis346BiasCF)
        if isinstance(getattr(DAVIS.Davis346BiasCF, attr), DAVIS.Davis346BiasCF)
    ]
